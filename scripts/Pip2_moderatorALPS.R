...
# load the data
dat_use <- rio::import("inputs/Pip2_subjectData.xlsx") 
...
############################ export the modulation effect #####################
mod_model <- bruceR::PROCESS(data = dat_mdd, y = "HAMD_item8", x = "log_Cprotindemean", mods = "ALPSdemean", 
                mod1.val = seq.int(from = -0.36, to = 0.27, by = 0.025),
                covs = c("age","gender"))
mod_result <- mod_model$results[[1]]$simple.slopes

rio::export(mod_result, file = "onputs/Pip3_ALPSmodRetardation.xlsx")
